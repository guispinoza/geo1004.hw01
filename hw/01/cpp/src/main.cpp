    ...
    if (stream_in.is_open()) {
        ...
        while (getline(stream_in, line)) {
            ...
            if (word == "f") {
                ...
                while (iss >> word) index.push_back(std::stoi(word)-1); //stoi converts str to int // we also substract 1 bc obj starts at 1 and C++ starts at 0 indexing
...
//        Point vd = vertices[(f.d)].point;
    ...
    for (auto i:darts){
        ...
        output_darts<<i.a2<< ";";
        output_darts<<i.a3<< ";";
        output_darts<<(i.v)+1<< ";"; // adding +1 so that the vertices id start from 1 and not for zero. in the main we used as the id the index, and indexes were starting from 0
        output_darts<<i.e<< ";";
        output_darts<<i.f;
        output_darts << "\n";
    }
    ...
    for (auto i : e_map){
        int count_dart = 0;
        for (auto x : darts) {
            if (x.e == i.second && count_dart ==0) { //we dont need to add 1, because in the main we stored the indices in both cases
                count_dart++;
                output_edges << (i.second)+1 << ";"; // adding +1 so that the vertices id start from 1 and not for zero. in the main we used as the id the index, and indexes were starting from 0
    ...
    for (auto i:f_map) {
        ...
        for (auto x:darts) {
            if (x.f==i.second && count_dart ==0) { //x.f is the face id that was inserted in a dart and i.second is the unique id that was added in each face as a value
                count_dart++;
    ...
    output_volume<<"1"<< ";"; //because we only have 1 object then the volume would have 1 object and we chose randomly one of the darts
    output_volume<<"1";
    output_volume<< "\n";
